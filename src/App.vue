<template>
  ...
  <VueFlow :nodes="nodes" :edges="edges" :nodes-draggable="false">
    <Panel>
      ...
      <div class="form-ver">
        <input type="text" v-model="formState.state.pathSearch.startVer" placeholder="Начальная вершина" maxLength="1" />
        <input type="text" v-model="formState.state.pathSearch.endVer" placeholder="Конечная вершина" maxLength="1" />
...
const appendVer = (edgeSymbol: String | null) => {
  ...
  const newVerTitle = edgeSymbol == null 
    ? lastElement.data.label.charCodeAt() + 1 
    : edgeSymbol.charCodeAt(0)
  const newVerCoordX = length % 5 == 0 
    ? 100
    : parseInt(lastElement.position.x) + STEP
  const newVerCoordY = length % 5 == 0 
    ? parseInt(lastElement.position.y) + STEP
    : parseInt(lastElement.position.y)
...
const createEdge = () => {
  ...
  let inputVerIndex = nodes.value.findIndex(item => item.data.label == formState.state.newEdge.inputVer)
  if (inputVerIndex == -1) {
    inputVerIndex = appendVer(formState.state.newEdge.inputVer) - 1
  }


  let outputVerIndex = nodes.value.findIndex(item => item.data.label == formState.state.newEdge.outputVer)
  if (outputVerIndex == -1) {
...
const findRoute = () => {
  ...
  const startVer = nodes.value.find((item: Node) => item.data.label == formState.state.pathSearch.startVer)?.id
  const verEnd = nodes.value.find((item: Node) => item.data.label == formState.state.pathSearch.endVer)?.id
  ...
  alert(`Итоговый результат ${formState.state.pathSearch.startVer} -> ${formState.state.pathSearch.endVer}: ${result}`)
